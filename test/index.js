...
describe("Plugin", () => {
  describe("File test", () => {
    ...
    test("parses setup with TypeScript and returns true", () => {
      const filename = path.join(__dirname, "./setup-ts/input.vue");
      expect(fileTest(filename)).toBe(true);
    });

    test("parses functional template and returns false", () => {
      const filename = path.join(__dirname, "./functional/input.vue");
      expect(fileTest(filename)).toBe(false);
    });
  });
});
